...
func NewRequestID(value string) (RequestID, error) {
	if !boundedValue(value, MaxRequestIDBytes) {
		return RequestID{}, fmt.Errorf("invalid request id")
	}
	return RequestID{value: value}, nil
}
func NewAttemptID(value string) (AttemptID, error) {
	if !boundedValue(value, MaxAttemptIDBytes) {
		return AttemptID{}, fmt.Errorf("invalid attempt id")
	}
	return AttemptID{value: value}, nil
}
func NewTenantScope(value string) (TenantScope, error) {
	if !boundedValue(value, MaxTenantScopeBytes) {
		return TenantScope{}, fmt.Errorf("invalid tenant scope")
	}
	return TenantScope{value: value}, nil
}
func NewModelKey(value string) (ModelKey, error) {
	if !boundedValue(value, MaxModelKeyBytes) {
		return ModelKey{}, fmt.Errorf("invalid model key")
	}
	return ModelKey{value: value}, nil
}
func NewSecretString(value []byte) (SecretString, error) {
	if len(value) == 0 || len(value) > MaxSecretBytes {
		return SecretString{}, fmt.Errorf("invalid secret")
	}
	return SecretString{value: append([]byte(nil), value...)}, nil
}
func NewSecretStringFromString(value string) (SecretString, error) {
	return NewSecretString([]byte(value))
}
...
func NewFeatureSet(version FeatureVersion, bits uint64) (FeatureSet, error) {
	if version != FeatureVersion1 || bits&^knownFeatureBits != 0 {
		return FeatureSet{}, fmt.Errorf("invalid feature set")
	}
...
func (s FeatureSet) Valid() bool {
	return s.version == FeatureVersion1 && s.bits&^knownFeatureBits == 0
}
func (s FeatureSet) Contains(required FeatureSet) bool {
	return s.Valid() && required.Valid() && s.version == required.version && s.bits&required.bits == required.bits
}
...
func validCachePolicy(v CachePolicy) bool {
	return v >= CachePolicyDisabled && v <= CachePolicyRequireCompatible
}
...
func NewPublicError(kind ErrorKind) error {
	if kind < ErrorInvalidRequest || kind > ErrorInternal {
		kind = ErrorInternal
	}
...
func NewInput(p InputParams) (Input, error) {
	if len(p.Messages) == 0 || len(p.Messages) > 128 || len(p.Extensions) > 32 {
	...
	for i, m := range p.Messages {
		...
		if m.Content == "" || len(m.Content) > 1<<20 || !utf8.ValidString(m.Content) {
			return Input{}, fmt.Errorf("invalid input")
		}
	...
	for k, v := range p.Extensions {
		if !boundedValue(k, 64) || len(v) > 4096 || !utf8.ValidString(v) {
			return Input{}, fmt.Errorf("invalid input")
		}
		extensions[k] = v
	}
...
func NewInferenceRequest(p InferenceRequestParams) (InferenceRequest, error) {
	...
	if model.value == "" {
		var err error
		model, err = NewModelKey(strings.TrimSpace(p.Model))
		if err != nil {
	...
	if len(input.messages) == 0 {
		var err error
		input, err = NewInput(InputParams{Messages: p.Messages})
		if err != nil {
			return InferenceRequest{}, NewPublicError(ErrorInvalidRequest)
		}
	} else {
	...
	if !validPriority(priority) || !features.Valid() || !validCachePolicy(cachePolicy) || budget <= 0 || budget > 30*time.Minute {
		return InferenceRequest{}, NewPublicError(ErrorInvalidRequest)
	}
...
func (r InferenceRequest) IdempotencyKey() (SecretString, bool) {
...
func NewPrincipal(tenant string, models []string) (Principal, error) {
	t, err := NewTenantScope(strings.TrimSpace(tenant))
	if err != nil {
		return Principal{}, fmt.Errorf("invalid principal")
	}
	keys := make([]ModelKey, len(models))
	for i, model := range models {
		keys[i], err = NewModelKey(strings.TrimSpace(model))
		if err != nil {
			return Principal{}, fmt.Errorf("invalid principal")
		}
	}
...
func NewPrincipalFromParams(p PrincipalParams) (Principal, error) {
	if !boundedValue(p.Subject, 256) || p.Tenant.value == "" || len(p.Models) == 0 || len(p.Models) > 128 || !p.Features.Valid() || !validPriority(p.MaxPriority) || len(p.CachePolicies) == 0 {
		return Principal{}, fmt.Errorf("invalid principal")
	}
	models := make(map[ModelKey]struct{}, len(p.Models))
	...
	var policies uint8
	for _, policy := range p.CachePolicies {
		if !validCachePolicy(policy) {
			return Principal{}, fmt.Errorf("invalid principal")
		}
		policies |= 1 << policy
	}
...
func NewCheckedAuthorizedRequest(p Principal, r InferenceRequest) (AuthorizedRequest, error) {
	if !p.Allows(r) {
		return AuthorizedRequest{}, NewPublicError(ErrorForbidden)
	}
...
func (u Usage) Valid() bool {
	return uint64(u.PromptTokens)+uint64(u.CompletionTokens) == uint64(u.TotalTokens)
}

type OutputDelta struct{ data []byte }

func NewOutputDelta(data []byte) (OutputDelta, error) {
	if len(data) == 0 || len(data) > MaxOutputDeltaBytes || !utf8.Valid(data) {
		return OutputDelta{}, fmt.Errorf("invalid output delta")
	}
...
func (p TerminalProof) Valid() bool {
	return p >= TerminalProofProviderFinish && p <= TerminalProofAuthenticatedProviderTermination
}
...
func NewDeltaEvent(delta string) (StreamEvent, error) {
	d, err := NewOutputDelta([]byte(delta))
	if err != nil {
		return StreamEvent{}, err
	}
	return NewStreamEvent(StreamEventParams{Kind: StreamEventDelta, Delta: d})
}
func NewUsageEvent(usage Usage) (StreamEvent, error) {
	return NewStreamEvent(StreamEventParams{Kind: StreamEventUsage, Usage: usage, HasUsage: true})
}
func NewTerminalEvent() StreamEvent {
	event, _ := NewStreamEvent(StreamEventParams{Kind: StreamEventTerminal, TerminalProof: TerminalProofProviderFinish})
	return event
}
func NewTerminalEventWithProof(proof TerminalProof) (StreamEvent, error) {
	return NewStreamEvent(StreamEventParams{Kind: StreamEventTerminal, TerminalProof: proof})
}
...
func (e StreamEvent) TerminalProof() (TerminalProof, bool) {
	return e.terminalProof, e.kind == StreamEventTerminal && e.terminalProof.Valid()
}

type CompletionResultParams struct {
	Output        []byte
	Usage         Usage
	HasUsage      bool
	TerminalProof TerminalProof
...
func NewCompletionResult(p CompletionResultParams) (CompletionResult, error) {
	if len(p.Output) > MaxOutputDeltaBytes || !utf8.Valid(p.Output) || !p.TerminalProof.Valid() || p.TerminalProof == TerminalProofNotSent || (p.HasUsage && !p.Usage.Valid()) || (!p.HasUsage && p.Usage != (Usage{})) {
		return CompletionResult{}, fmt.Errorf("invalid completion result")
	}
...
func (r CompletionResult) Complete() bool {
	return r.terminalProof.Valid() && r.terminalProof != TerminalProofNotSent
}
