...
func TestMoverHandleIsCopiedBoundedAndRedacted(t *testing.T) {
	...
	spec, err := NewTransferSpec(TransferSpecParams{Tenant: "tenant-a", RequestID: "request-a", CacheIdentity: id, Source: src, Destination: dst, SourceManifest: m, DestinationManifest: m, ExpiresAt: time.Now().Add(time.Minute), MaxBytes: 1024})
	if err != nil {
		t.Fatal(err)
	}
...
func TestTransferStatusIsMonotonic(t *testing.T) {
	...
	src := testWorkload(t, "pod-a", 1)
	dst := testWorkload(t, "pod-b", 1)
	spec, _ := NewTransferSpec(TransferSpecParams{Tenant: "tenant-a", RequestID: "request-a", CacheIdentity: id, Source: src, Destination: dst, SourceManifest: m, DestinationManifest: m, ExpiresAt: time.Now().Add(time.Minute), MaxBytes: 1024})
