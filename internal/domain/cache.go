...
func NewReservedTarget(tenant string, identity WorkloadIdentity, manifest CapabilityManifest, cacheIdentity CacheIdentity) (ReservedTarget, error) {
	if !boundedProviderString(tenant, 128) || identity.PodUID() == "" || cacheIdentity.Tenant() != tenant || !manifest.Supports(CapabilityInference) || manifest.ImageDigest() != cacheIdentity.ProviderImageDigest() || manifest.ProxyDigest() != cacheIdentity.ProxyDigest() || manifest.ID() != cacheIdentity.ManifestID() {
...
func NewTransferSpec(p TransferSpecParams) (TransferSpec, error) {
	if !boundedProviderString(p.Tenant, 128) || !boundedProviderString(p.RequestID, 256) || p.CacheIdentity.Tenant() != p.Tenant || p.Source.PodUID() == "" || p.Destination.PodUID() == "" || p.Source.Equal(p.Destination) || !p.SourceManifest.Supports(CapabilityMover) || !p.DestinationManifest.Supports(CapabilityMover) || !p.SourceManifest.Compatible(p.DestinationManifest) || p.SourceManifest.ID() != p.CacheIdentity.ManifestID() || p.SourceManifest.ImageDigest() != p.CacheIdentity.ProviderImageDigest() || p.ExpiresAt.IsZero() || p.MaxBytes == 0 {
...
func NewMoverHandle(spec TransferSpec, opaque []byte) (MoverHandle, error) {
	...
	return MoverHandle{append([]byte(nil), opaque...), spec.tenant, spec.requestID, spec.cacheIdentity, spec.source, spec.destination, spec.sourceManifest.ID(), spec.expiresAt}, nil
}
...
func NewTransferStatus(p TransferStatusParams) (TransferStatus, error) {
	if len(p.Handle.opaque) == 0 || p.Sequence == 0 || !validTransferState(p.State) {
