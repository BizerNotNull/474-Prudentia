...
func NewCapabilityManifest(p CapabilityManifestParams) (CapabilityManifest, error) {
	...
	if err != nil || !containsString(routes, "/v1/chat/completions") {
		return CapabilityManifest{}, fmt.Errorf("invalid capability manifest routes")
	}
	fields, err := validatedUniqueStrings(p.Fields, 64, 128)
	if err != nil {
		return CapabilityManifest{}, fmt.Errorf("invalid capability manifest fields")
	}
	...
	case APCDisabled, APCTenantDedicated:
		if p.TenantSaltVersion != 0 || p.TenantSaltProven {
			return CapabilityManifest{}, fmt.Errorf("invalid APC isolation")
		}
	case APCTenantSalted:
		if p.TenantSaltVersion != 1 || !p.TenantSaltProven {
			return CapabilityManifest{}, fmt.Errorf("unproven tenant salt")
		}
	default:
...
func (m CapabilityManifest) TenantSaltVersion() (uint16, bool) {
	return m.tenantSaltVersion, m.apcIsolation == APCTenantSalted
}
...
func (m CapabilityManifest) Supports(c ProviderCapability) bool {
	if m.schemaVersion != CurrentManifestSchemaVersion || m.capabilityVersion != CurrentCapabilityVersion || m.signatureVersion != CurrentSignatureVersion {
		return false
	}
	switch c {
	case CapabilityInference:
		return true
	case CapabilityMetrics:
		return len(m.metrics) != 0
	case CapabilityAPC:
		return m.apcIsolation == APCTenantDedicated || (m.apcIsolation == APCTenantSalted && m.tenantSaltVersion == 1)
	case CapabilityTermination:
		return m.termination
	case CapabilityCacheMetadata:
		return m.cacheMetadata
	case CapabilityMover:
		return m.mover
	default:
		return false
	}
}
func (m CapabilityManifest) ValidAt(at time.Time) bool {
...
func (c BackendCall) Request() AuthorizedRequest   { return c.request }
func (c BackendCall) Target() DispatchTarget       { return c.target }
func (c BackendCall) Manifest() CapabilityManifest { return c.manifest }
func (c BackendCall) ProviderRequestID() string    { return c.providerRequestID }
...
func NewProbeTarget(target DispatchTarget, manifest CapabilityManifest) (ProbeTarget, error) {
	if target.Endpoint() == "" || !manifest.Supports(CapabilityInference) {
		return ProbeTarget{}, fmt.Errorf("invalid probe target")
	}
	return ProbeTarget{target, manifest}, nil
}
func (t ProbeTarget) Target() DispatchTarget       { return t.target }
func (t ProbeTarget) Manifest() CapabilityManifest { return t.manifest }

type RuntimeHealthState uint8

const (
	RuntimeHealthResponsive RuntimeHealthState = iota + 1
	RuntimeHealthUnresponsive
	RuntimeHealthWarming
)
...
func NewRuntimeHealthObservation(i WorkloadIdentity, s RuntimeHealthState, at time.Time) (RuntimeHealthObservation, error) {
	if i.PodUID() == "" || at.IsZero() || (s != RuntimeHealthResponsive && s != RuntimeHealthUnresponsive && s != RuntimeHealthWarming) {
		return RuntimeHealthObservation{}, fmt.Errorf("invalid runtime health observation")
	}
	return RuntimeHealthObservation{i, s, at}, nil
}
func (o RuntimeHealthObservation) Identity() WorkloadIdentity { return o.identity }
func (o RuntimeHealthObservation) State() RuntimeHealthState  { return o.state }
func (o RuntimeHealthObservation) ObservedAt() time.Time      { return o.observedAt }

type LoadObservationParams struct {
	Identity      WorkloadIdentity
	ObservedAt    time.Time
	UsedSlots     uint32
	HasUsedSlots  bool
...
func NewLoadObservation(p LoadObservationParams) (LoadObservation, error) {
	if p.Identity.PodUID() == "" || p.ObservedAt.IsZero() || (!p.HasUsedSlots && p.UsedSlots != 0) || (!p.HasQueueDepth && p.QueueDepth != 0) {
		return LoadObservation{}, fmt.Errorf("invalid load observation")
	}
	return LoadObservation{p.Identity, p.ObservedAt, p.UsedSlots, p.HasUsedSlots, p.QueueDepth, p.HasQueueDepth}, nil
}
func (o LoadObservation) Identity() WorkloadIdentity { return o.identity }
func (o LoadObservation) ObservedAt() time.Time      { return o.observedAt }
func (o LoadObservation) UsedSlots() (uint32, bool)  { return o.usedSlots, o.hasUsedSlots }
func (o LoadObservation) QueueDepth() (uint32, bool) { return o.queueDepth, o.hasQueueDepth }

type ProviderRequestRefParams struct {
	Tenant, RequestID, ProviderRequestID string
	Target                               WorkloadIdentity
	Manifest                             CapabilityManifest
	ExpiresAt                            time.Time
}
type ProviderRequestRef struct {
	tenant, requestID, providerRequestID string
	target                               WorkloadIdentity
	manifest                             CapabilityManifest
	expiresAt                            time.Time
...
func NewProviderRequestRef(p ProviderRequestRefParams) (ProviderRequestRef, error) {
	if !boundedProviderString(p.Tenant, 128) || !boundedProviderString(p.RequestID, 256) || !boundedProviderString(p.ProviderRequestID, 256) || p.Target.PodUID() == "" || p.ExpiresAt.IsZero() || !p.Manifest.Supports(CapabilityTermination) {
		return ProviderRequestRef{}, fmt.Errorf("provider termination unsupported or invalid")
	}
	return ProviderRequestRef{p.Tenant, p.RequestID, p.ProviderRequestID, p.Target, p.Manifest, p.ExpiresAt}, nil
}
func (r ProviderRequestRef) Tenant() string               { return r.tenant }
func (r ProviderRequestRef) RequestID() string            { return r.requestID }
func (r ProviderRequestRef) ProviderRequestID() string    { return r.providerRequestID }
func (r ProviderRequestRef) Target() WorkloadIdentity     { return r.target }
func (r ProviderRequestRef) Manifest() CapabilityManifest { return r.manifest }
func (r ProviderRequestRef) ExpiresAt() time.Time         { return r.expiresAt }
func (r ProviderRequestRef) String() string               { return "provider-request[redacted]" }
...
func boundedProviderString(v string, max int) bool {
	return v != "" && len(v) <= max && v == strings.TrimSpace(v)
}
func validatedUniqueStrings(in []string, maxCount, maxLen int) ([]string, error) {
	if len(in) == 0 {
		return nil, fmt.Errorf("empty")
	}
	return validatedUniqueStringsAllowEmpty(in, maxCount, maxLen)
}
func validatedUniqueStringsAllowEmpty(in []string, maxCount, maxLen int) ([]string, error) {
	if len(in) > maxCount {
	...
	for _, v := range out {
		if !boundedProviderString(v, maxLen) {
