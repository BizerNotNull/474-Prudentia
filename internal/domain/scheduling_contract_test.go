...
func TestScheduleCommandNoKeyStillRequiresDigests(t *testing.T) {
	...
	if err != nil { t.Fatalf("no-key command rejected: %v", err) }
	if command.HasIdempotencyKey() || len(command.DigestCandidates()) != 1 { t.Fatal("no-key candidate shape changed") }
	missing := valid; missing.DigestCandidates = nil; missing.DigestWriteVersion = 0
	if _, err := NewScheduleCommand(missing); err == nil { t.Fatal("no-key command without digest candidates accepted") }
	mutated := command.DigestCandidates(); mutated[0] = RequestDigestCandidate{}
	if command.DigestCandidates()[0].Version() != 2 { t.Fatal("digest accessor exposed backing slice") }
}
...
func TestScheduleCommandRejectsUnsortedAndUnknownFeatures(t *testing.T) {
	...
	if _, err := NewScheduleCommand(params); err == nil { t.Fatal("unsorted digest versions accepted") }
	params.DigestCandidates = []RequestDigestCandidate{d1}; params.DigestWriteVersion = 1; params.Features = FeatureSet{version: 99}
	if _, err := NewScheduleCommand(params); err == nil { t.Fatal("unknown feature version accepted") }
}
...
func TestReservationRefCopiesAndRedactsCapability(t *testing.T) {
	...
	if err != nil { t.Fatal(err) }
	capability[0] = 'X'
	copyBytes := ref.Capability(); copyBytes[1] = 'Y'
	if ref.Capability()[0] != 's' || ref.Capability()[1] != 's' { t.Fatal("reservation capability changed through alias") }
	if strings.Contains(fmt.Sprintf("%v %#v", ref, ref), strings.Repeat("s", 8)) { t.Fatal("reservation formatting exposed capability") }
}
...
func TestSnapshotAndCatalogCopyCacheHints(t *testing.T) {
	...
	if err != nil { t.Fatal(err) }
	hints[0] = CacheHint{}
	accessorHints := snapshot.CacheHints(); accessorHints[0] = CacheHint{}
	if snapshot.CacheHints()[0].Digest() == ([32]byte{}) { t.Fatal("snapshot cache hints changed through alias") }
	catalog, err := NewCandidateCatalog([]InstanceSnapshot{snapshot}, asOf)
	if err != nil { t.Fatal(err) }
	candidates := catalog.Candidates(); candidates[0].cacheHints[0] = CacheHint{}
	if catalog.Candidates()[0].CacheHints()[0].Digest() == ([32]byte{}) { t.Fatal("catalog changed through nested alias") }
}
